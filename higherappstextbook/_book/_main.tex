...
Initially a consideration of a possible relationship should be \emph{descriptive}, thinking about when the dependent variable might be increasing slowly, increasing more quickly, decreasing, at zero or other specific values.
...
Some types of relationship are commonly encountered, and the rest of this chapter will study these in more detail. One of the simplest relationships between two variables is a \emph{linear relationship}, which on a graph is a \emph{straight line}.
...
For an equation of the form \(y=a+bx\) describing a linear relationship between \(x\) and \(y\), the value of \(a\) tells us the \emph{starting value} of \(y\) (when \(x=0\)), and the value of \(b\) tells us how much \(y\) changes for each `step' in the \(x\) direction.
